...
public class MovieProvider extends ContentProvider {
    ...
    public String getType(Uri uri) {
        ...
        switch (match) {
            case MOVIE:
                return MovieContract.MovieEntity.CONTENT_TYPE;
            default:
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (sUriMatcher.match(uri)) {
            case MOVIE: {
                ...
                if (_id > 0) {
                    returnUri = MovieContract.MovieEntity.appendMovieId(_id);
                }
                else {
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            case MOVIE:
                rowsUpdated = db.update(MovieContract.MovieEntity.TABLE_NAME, values, selection,
                        selectionArgs);
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        ...
        switch (sUriMatcher.match(uri)) {
            case MOVIE: {
                retCursor = mOpenHelper.getReadableDatabase().query(
                        MovieContract.MovieEntity.TABLE_NAME, projection, selection, selectionArgs,
                        null, null, sortOrder);
                break;
            }
            default:
